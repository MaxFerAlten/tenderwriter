...
from app.utils.naming import get_structured_minio_path
        ...
        def _cleanup_sync():
            ...
            for obj in objects:
                ...
                if last_modified and last_modified.astimezone(timezone.utc) < cutoff_time:
                    ...
                    deleted_count += 1
                    logger.info(
                        "Old document deleted", doc_key=obj.object_name, last_modified=last_modified
                    )
...
@router.get(
    "/document/proposal/{proposal_id}/{section_id}", response_model=OnlyOfficeConfigResponse
)
async def get_document_config(
    ...
    result = await db.execute(
        ...
        .options(selectinload(ProposalSection.proposal).selectinload(Proposal.tender))
    )
    ...
    doc_key = _generate_document_key(
        proposal_id, section_id, section.updated_at or section.created_at
    )
...
async def get_tender_document_config(
    ...
    doc_key = _generate_library_document_key(
        doc_entity.id, doc_entity.updated_at or doc_entity.created_at
    )
...
async def onlyoffice_callback(
    ...
    if payload.status in (2, 6) and payload.url:
        ...
        if not metadata:
            logger.warning(
                "Unknown document key in callback (Redis lookup failed)", key=payload.key
            )
            ...
            if is_section:
                ...
                if extracted_text.strip():
                    try:
                        ...
                        logger.info(
                            "Section content indexed in RAG", section_id=section_id, stats=stats
                        )
                    except Exception as e:
                        logger.error(
                            "Failed to index section in RAG", error=str(e), section_id=section_id
                        )
                ...
                if extracted_text.strip():
                    try:
                        ...
                        logger.error(
                            "Failed to index library block in RAG", error=str(e), block_id=block_id
                        )
...
async def force_save_library(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Force save failed: {str(e)}") from e
