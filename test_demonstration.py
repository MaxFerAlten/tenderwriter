...
from backend.app.api.tenders import delete_tender
...
async def test_bug_01_delete_tender_commits():
    ...
    with patch("backend.app.api.tenders.check_tender_access", new_callable=AsyncMock) as mock_check:
...
from backend.app.auth.legacy import LegacyJWTProvider
...
async def test_bug_16_inactive_or_unverified_jwt_rejected():
    ...
    with patch("backend.app.auth.legacy.jwt.decode") as mock_jwt:
...
from backend.app.auth.provider import _get_provider, get_current_user as dep_get_current_user
from backend.app.auth.legacy import LegacyJWTProvider
