...
### 4. Persistenza Status Ingestion e Raffinamento UI
Abbiamo risolto il problema della sparizione del tasto "Activate Tender" e raffinato il comportamento multi-documento:
- **Backend**: Esteso il modello `TenderResponse` per includere il campo `ingestion_status`, calcolato dinamicamente dallo stato dei documenti nel DB.
- **Frontend**: Il tasto **Upload PDF** rimane sempre visibile nei Draft per permettere caricatori multipli.
- **Frontend**: Il tasto **Activate Tender** compare separatamente e viene disabilitato automaticamente se c'è un'ingestion (`processing`) in corso per qualsiasi documento del tender.
